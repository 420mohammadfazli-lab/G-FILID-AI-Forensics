...
st.markdown("""
    ...
    .agency-logo {
        ...
        padding: 5px; box-shadow: 0 0 40px rgba(212, 175, 55, 0.5);
    }
    ...
    .main-title {
        ...
        letter-spacing: 3px; text-shadow: 0 0 15px rgba(212, 175, 55, 0.4);
    }
    ...
    <div class="status-bar">OFFICIAL GOVERNMENT COMMUNICATION - ENCRYPTION ACTIVE [AES-256]</div>
...
# --- 5. NAVIGATION ---
tab1, tab2 = st.tabs(["🏛️ FIAT AUDIT (GOVERNMENT)", "₿ BLOCKCHAIN INTELLIGENCE (CRYPTO)"])

# --- TAB 1: TRADITIONAL FINANCIAL AUDIT ---
with tab1:
    st.subheader("📁 SYSTEM INPUT: FINANCIAL DOSSIER")
    uploaded_file = st.file_uploader("UPLOAD SOURCE FILE (SECURE CSV/XLSX)", type=["csv", "xlsx"], key="fiat_upload")
        ...
        with st.spinner("💠 DECRYPTING NEURAL PACKETS..."):
            ...
            try:
                ...
                if len(selected_cols) >= 2:
                    ...
                    st.subheader("🚩 BLACKLISTED ENTITIES")
                    blacklist = df[df['Anomaly_Score'] == -1].drop(columns=['Anomaly_Score'])
                    st.dataframe(blacklist, use_container_width=True)
                else:
                    st.warning("SYSTEM NOTICE: MINIMUM 2 PARAMETERS REQUIRED FOR AI SCAN.")
            except Exception as e:
                st.error(f"CRITICAL ERROR: {e}")
    else:
        st.info("AWAITING SECURE DATA UPLOAD...")

# --- TAB 2: BLOCKCHAIN INTELLIGENCE ---
with tab2:
    st.subheader("🌐 REAL-TIME BLOCKCHAIN SURVEILLANCE")
    st.markdown("Enter a Bitcoin (BTC) wallet address to trace transactions and assess risk.")
    
    wallet_address = st.text_input("ENTER BTC WALLET ADDRESS (e.g., 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa):")
    
    if wallet_address:
        with st.spinner("📡 SCANNING GLOBAL LEDGER..."):
            try:
                # API Call to Blockchain.info
                ...
                if response.status_code == 200:
                    ...
                    mc1.metric("CURRENT BALANCE", f"{balance:.4f} BTC")
                    mc2.metric("TOTAL TRANSACTIONS", n_tx)
                    mc3.metric("TOTAL SENT", f"{total_sent:.4f} BTC")
                    
                    # Risk Assessment Logic
                    st.markdown("#### 🧠 AI RISK ASSESSMENT")
                    if n_tx > 500:
                        st.error("🚨 ALERT: HIGH-FREQUENCY ACTIVITY DETECTED (POTENTIAL MIXER/LAUNDERING)")
                    elif balance > 100:
                        st.warning("⚠️ NOTICE: WHALE ACCOUNT DETECTED - MONITORING FOR MARKET MANIPULATION")
                    else:
                        st.success("✅ STATUS: COMPLIANT / NORMAL ACTIVITY")
                    ...
                    for tx in data['txs'][:10]:
                        txs.append({
                            ...
                            "RESULT_BTC": tx['result'] / 100000000
                        })
                    st.table(pd.DataFrame(txs))
                else:
                    st.error("SYSTEM ERROR: INVALID WALLET ADDRESS OR NETWORK TIMEOUT.")
            except Exception as e:
                st.error(f"CONNECTION ERROR: {e}")
    else:
        st.info("SYSTEM STATUS: WAITING FOR WALLET IDENTIFIER...")

# --- 6. SIDEBAR AGENT AUTHENTICATION ---
st.sidebar.divider()
